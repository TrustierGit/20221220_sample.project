...
CREATE TABLE `holidays` (
  ...
  `flag_delete` int(11) NOT NULL DEFAULT 0 COMMENT '削除フラグ（0:アクティブ/1:削除）',
...
CREATE TABLE `notifications` (
  `id` bigint(20) UNSIGNED NOT NULL,
  `flag_display` int(11) NOT NULL COMMENT '全体周知系',
  `domain_organization` varchar(100) COLLATE utf8mb4_bin NOT NULL COMMENT '自治体ドメイン',
  `mode_reserve` int(11) NOT NULL DEFAULT 0 COMMENT 'アカウントフラグ（0:通常/1:常時）',
...
(3, 0, 'marumaru.co.jp', 0, '2023-01-01', 'Info', '年末年始休業の為予約不可', '2022-12-16 02:34:51', '2022-12-16 02:34:51');
...
CREATE TABLE `organizations` (
  ...
  `count_license` int(11) NOT NULL DEFAULT 0 COMMENT '所持ライセンス',
  `date_maintenance` date NOT NULL COMMENT 'メンテナンス',
  `name_file` varchar(100) COLLATE utf8mb4_bin NOT NULL COMMENT 'csvファイル名',
  `flag_license` int(11) NOT NULL DEFAULT 0 COMMENT '契約形態（0:一般/1:個別）',
  `flag_delete` int(11) NOT NULL DEFAULT 0 COMMENT '削除フラグ（0:アクティブ/1:削除）',
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
(2, 'App\\Models\\User', 1, 'test', 'b36d8353898a1a22e3317d7e3be54977ca4af4d6ca5923d4d83405f04e548020', '[\"*\"]', '2023-01-11 00:03:31', NULL, '2022-12-28 05:28:27', '2023-01-11 00:03:31');
...
CREATE TABLE `reservations` (
  ...
  `text_remarks` text COLLATE utf8mb4_bin DEFAULT NULL COMMENT '備考欄',
...
(4, 'marumaru.co.jp', 1, '2023-01-01', 'user3@marumaru.co.jp', NULL, '2022-12-16', '2022-12-28 06:38:27'),
(5, 'marumaru.co.jp', 1, '2023-01-01', 'user4@marumaru.co.jp', NULL, '2022-12-16', '2022-12-28 06:38:38'),
(6, 'marumaru.co.jp', 1, '2023-01-01', 'user5@marumaru.co.jp', NULL, '2022-12-16', '2022-12-28 06:38:44'),
(7, 'marumaru.co.jp', 1, '2023-01-01', 'user6@marumaru.co.jp', NULL, '2022-12-16', '2022-12-28 06:38:58'),
(9, 'marumaru.co.jp', 1, '2023-01-01', 'user1@marumaru.co.jp', NULL, '2022-12-16', '2022-12-28 06:39:03'),
...
(27, 'marumaru.co.jp', 0, '2023-01-01', 'user@marumaru.co.jp', NULL, '2022-12-28', '2022-12-28 06:39:23'),
(30, 'marumaru.co.jp', 0, '2023-09-13', 'user@marumaru.co.jp', NULL, '2023-09-04', '2023-09-04 04:26:38'),
(32, 'marumaru.co.jp', 0, '2023-01-12', 'user@marumaru.co.jp', NULL, '2023-01-10', '2023-01-10 23:50:46');
...
CREATE TABLE `users` (
  ...
  `mode_admin` tinyint(4) NOT NULL COMMENT '管理者モード(0:一般/1:自治体/9:NESIC)',
  `flag_delete` int(11) NOT NULL DEFAULT 0 COMMENT '削除フラグ（0:アクティブ/1:削除）',
...
(1, 'super@marumaru.co.jp', 'marumaru.co.jp', 0, 'superuser', '$2y$10$0/CWIK35p1h6EUleady7Vebc1X6kGUU3iKk4nJ/30Ol1kDOrsw.Ta', 9, 0, NULL, '4g2vHp1djwKM0RhfiwYPsLQVoKKgPu29gbqPEkwD', NULL, NULL),
...
(10, 'user@test3.co.jp', 'test3.co.jp', 0, 'user', '$2y$10$f0eu3xGKc10as4xqSZ94yeA0TlFBDJ/RBNErt8KQb.055xbEg6UaG', 0, 0, NULL, NULL, NULL, NULL);

-- --------------------------------------------------------

--
-- ビュー用の代替構造 `user_organization`
-- (実際のビューを参照するには下にあります)
--
CREATE TABLE `user_organization` (
`domain_organization` varchar(30)
,`mode_reserve` int(11)
...
--
-- ビュー用の代替構造 `ドメインがないぞビュー`
-- (実際のビューを参照するには下にあります)
--
CREATE TABLE `ドメインがないぞビュー` (
`email` varchar(255)
,`domain_organization` varchar(30)
);
...
--
-- ビュー用の構造 `user_organization`
--
DROP TABLE IF EXISTS `user_organization`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `user_organization`  AS SELECT `o`.`domain_organization` AS `domain_organization`, `o`.`mode_reserve` AS `mode_reserve`, `o`.`name_organization` AS `name_organization`, `u`.`email` AS `email` FROM (`organizations` `o` left join `users` `u` on(`o`.`domain_organization` = `u`.`domain_organization`))  ;
...
-- ビュー用の構造 `ドメインがないぞビュー`
--
DROP TABLE IF EXISTS `ドメインがないぞビュー`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ドメインがないぞビュー`  AS SELECT `u`.`email` AS `email`, ifnull(`o`.`domain_organization`,'ドメインがないよ～') AS `domain_organization` FROM (`users` `u` left join `organizations` `o` on(`u`.`domain_organization` = `o`.`domain_organization`)) WHERE `o`.`domain_organization` is nullnull  ;
...
-- ビュー用の構造 `予約リスト`
--
DROP TABLE IF EXISTS `予約リスト`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `予約リスト`  AS SELECT `r`.`domain_organization` AS `domain_organization`, `r`.`mode_reserve` AS `mode_reserve`, `r`.`date_reservation` AS `date_reservation`, `r`.`email_staff` AS `email_staff`, `o`.`stored_server` AS `stored_server`, `o`.`name_file` AS `name_file`, `o`.`flag_delete` AS `flag_delete` FROM (`reservations` `r` left join `organizations` `o` on(`r`.`domain_organization` = `r`.`domain_organization` and `r`.`mode_reserve` = `r`.`mode_reserve` and `o`.`flag_delete` = 0))  ;
